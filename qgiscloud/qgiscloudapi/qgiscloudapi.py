...
class API():
    ...
    def read_mapusers(self, map_name):
        ...
        """
        self.requires_auth()
        resource = '/maps/%s/users.json' % (map_name)
        request = Request(user=self.user, password=self.password, token=self.get_token(), cache=self.cache, url=self.url)
        content = request.get(resource)
        return json.loads(content)

    def create_mapuser(self, map_name, user):
        """
            Add a user to an map.
        """
        self.requires_auth()
        resource = '/maps/%s/users.json' % (map_name)
        request = Request(user=self.user, password=self.password, token=self.get_token(), cache=self.cache, url=self.url)
    ...
    def delete_mapuser(self, map_name, user_name):
        """
           Remove a user from an map.
        """
        self.requires_auth()
        resource = '/maps/%s/users/%s.json' % (map_name, user_name)
        request = Request(user=self.user, password=self.password, token=self.get_token(), cache=self.cache, url=self.url)
        content = request.delete(resource)
        return True
    ...
    def readuser(self, user_name):
        """
            Get user by user_name.
        """
        self.requires_auth()
        resource = '/users/%s.json' % (user_name)
        request = Request(user=self.user, password=self.password, token=self.get_token(), cache=self.cache, url=self.url)
        content = request.get(resource)
        return json.loads(content)

    def updateuser(self, user_name, activation_code=None):
        """
